...
With a **`sourcePath`** source the addressing depends on the source's shape: a 2-D array is addressed literally, exactly as an uploaded sheet is, while an array of objects gains a header row of field names at row 1 — so its first record is row **2**, and `=SUM(D2:D4)` totals the fourth field of three records.
