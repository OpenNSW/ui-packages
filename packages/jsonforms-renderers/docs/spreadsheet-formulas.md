...
Cell and range addressing is literal (`B2`, `I2:I6`), the same as in Excel — row 1 is always the sheet's first row, column A is always the sheet's first column. `columnHeader`/`rowHeader` and the mandatory, declared `columns`/`rows` field list together control what shape the persisted `sheet` itself ends up in (raw matrix vs. an array of records, keyed positionally by declared `id`) — see [spreadsheet-value-shape.md](./spreadsheet-value-shape.md).
